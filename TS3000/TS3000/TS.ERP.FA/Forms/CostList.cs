...
namespace TS.ERP.FA.Forms
{
    public partial class CostListForm : Form
    {
        ...
        {
            DataTable result = costService.GetDataByCondition(this.FilterCondition);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                BusinessControl.SetInfoByGrid(costInfo, this.gridMemberSalary);
                DialogResult diaResult = Msg.Show("是否删除单据[" + costInfo.cCode + "]？", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                if (diaResult == DialogResult.OK)
                {

                    costService.DoDel(costInfo);
        ...
        private void btnAudit_Click(object sender, EventArgs e)
        {
            try
            {
               // FunctionAccess.Access("btnAudit", this.GetType().Name);
                ...
                BusinessControl.SetInfoByGrid(costInfo, this.gridMemberSalary);
                DialogResult diaResult = Msg.Show("是否审核单据[" + costInfo.cCode + "]？", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                if (diaResult == DialogResult.OK)
                {

                    costService.DoAudit(costInfo);
        ...
        private void btnUnAudit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                BusinessControl.SetInfoByGrid(costInfo, this.gridMemberSalary);
                DialogResult diaResult = Msg.Show("是否反审核单据[" + costInfo.cCode + "]？", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                if (diaResult == DialogResult.OK)
                {

                    costService.DoUnAudit(costInfo);
        ...
        private void ShowCostDetailForm(Object rowIndex)
        {
            if (rowIndex != null)
            {
                CostDetailForm costForm = new CostDetailForm((int)rowIndex, this.gridMemberSalary.Rows);
                costForm.CtListForm = this;
                costForm.ShowDialog();
            }
            else
            {
                CostDetailForm costForm = new CostDetailForm("new");
                costForm.CtListForm = this;
                costForm.ShowDialog();
            }
        }  
